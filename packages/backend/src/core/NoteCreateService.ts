...
import { In, type DataSource, IsNull, LessThan } from "typeorm";
...
export class NoteCreateService implements OnApplicationShutdown {
	...
	private incRenoteCount(renote: MiNote) {
		...
		if (
			Math.random() < 0.3 &&
			Date.now() - this.idService.parse(renote.id).date.getTime() <
				1000 * 60 * 60 * 24 * 3
		) {
	...
	private async renderNoteOrRenoteActivity(data: Option, note: MiNote) {
		...
		const content =
			this.isRenote(data) && !this.isQuote(data)
	...
	private async pushToTl(
		...
		if (note.channelId) {
			...
			this.fanoutTimelineService.push(
				...
				note.id,
				note.userHost == null
					? meta.perLocalUserUserTimelineCacheMax
					: meta.perRemoteUserUserTimelineCacheMax,
				r,
			);
			...
			for (const channelFollowing of channelFollowings) {
				...
				if (note.fileIds.length > 0) {
					this.fanoutTimelineService.push(
						...
						note.id,
						meta.perUserHomeTimelineCacheMax / 2,
						r,
					);
				}
			}
			...
			for (const userListMembership of userListMemberships) {
				...
				this.fanoutTimelineService.push(
					...
					note.id,
					meta.perUserListTimelineCacheMax,
					r,
				);
				if (note.fileIds.length > 0) {
					this.fanoutTimelineService.push(
						...
						note.id,
						meta.perUserListTimelineCacheMax / 2,
						r,
					);
				}
			}
			...
			if (isReply(note)) {
				...
				if (note.fileIds.length > 0) {
					this.fanoutTimelineService.push(
						...
						note.id,
						note.userHost == null
							? meta.perLocalUserUserTimelineCacheMax / 2
							: meta.perRemoteUserUserTimelineCacheMax / 2,
						r,
					);
				}
	...
	public async checkHibernation(followings: MiFollowing[]) {
		...
		// ランダムに最大1000件サンプリング
		const samples = shuffle(followings).slice(
			0,
			Math.min(followings.length, 1000),
		);

		const hibernatedUsers = await this.usersRepository.find({
			where: {
				id: In(samples.map((x) => x.followerId)),
				lastActiveDate: LessThan(
	...
	public async checkProhibitedWordsContain(

...
export class NoteCreateService implements OnApplicationShutdown {
	...
	public async create(user: {
		...
		const willCauseNotification =
			mentionedUsers.filter((u) => u.host === null).length > 0 ||
			data.reply?.userHost === null ||
			data.renote?.userHost === null;

		if (
			this.config.misskeyBlockMentionsFromUnfamiliarRemoteUsers === true &&
			user.host !== null &&
			willCauseNotification
		) {
			...
			if ((userEntity?.followersCount ?? 0) === 0) {
				throw new Error(
					"Temporarily, notes including mentions, replies and renotes to local-user from remote users which is not followed by local-users are not allowed"
				);
			}
		}
